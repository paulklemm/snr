...
class OpenCPUBridge {
	...
	async runRCommand(rpackage, rfunction, params, valFormat = 'json') {
		...
		} catch (error) { timeStampLog(`Error in runRCommand: ${rpackage}, ${rfunction}, ${JSON.stringify(params)}, ${valFormat}`); timeStampLog(JSON.stringify(error, null, 2)); throw(error); }
		...
		return get(`${this.address}/ocpu`)
			.then((response) => {
				...
				if (response.status === 200) {
					...
					timeStampLog(`OpenCPU returns status ${response.status}, connection cannot be established`, true);
			})
			.catch((error) => {
